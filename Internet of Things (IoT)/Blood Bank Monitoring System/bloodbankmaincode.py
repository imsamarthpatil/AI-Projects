...
def check_temperature(value,checker):
	if value > conf.threshold[2]:
		...
		message = "Blood Bank Refrigerator Temperature increased the threshold value. The current value is: " + str(int(sensor_value*0.097))
	...
	if value < conf.threshold[0]:
		...
		message = "Blood Bank Refrigerator Temperature decreased below the threshold value. The current value is: " + str(int(sensor_value*0.097))
		telegram_status = send_telegram_message(message)
		print("This is the Telegram status", telegram_status)
		if not checker:
			return buzzer_alert()
	...
	if value > conf.threshold[1] and value < conf.threshold[2]:
		print("The temperature value is between ",str(int(conf.threshold[1]*0.097))," and ",str(int(conf.threshold[2]*0.097)), ". Sending an alert notification")
		message = "Blood Bank Refrigerator Temperature is between " + str(int(conf.threshold[1]*0.097)) + " and " + str(int(conf.threshold[2]*0.097)) + ". Check prediction table. The current value is: " + str(int(sensor_value*0.097))
		telegram_status = send_telegram_message(message)
		print("This is the Telegram status", telegram_status)
		if not checker:
			return buzzer_alert()
...
while True:
	...
	try:
		if sensor_value> bound[0]:
			...
			message = "Blood Bank Refrigerator Temperature increased suddenly. The current value is: " + str(int(sensor_value*0.097))
			telegram_status = send_telegram_message(message)
			print("This is the Telegram status", telegram_status)
			buzzer_alert()
			checker = True
			...
			message = "Blood Bank Refrigerator Temperature decreased suddenly. The current value is: " + str(int(sensor_value*0.097))
			telegram_status = send_telegram_message(message)
			print("This is the Telegram status", telegram_status)
			buzzer_alert()
			checker = True
